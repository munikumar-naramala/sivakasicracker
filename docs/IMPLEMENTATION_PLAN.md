...
- **`place-order.php`** — cart page. Two supported flows:
  1. **Minimal-change flow**: keep the "one big form for the whole catalog" pattern customers are used to, but generate every row from `Product::getAllVisible()` in a loop (not a 177-entry hand-written array) and stop rendering price as an editable-looking input — show it as read-only text, submit only `product_id` + `quantity`.
  2. **Recommended**: add real add-to-cart buttons on `price-list.php`/`products-show.php` that POST to `Cart`, so `place-order.php` becomes a normal cart review + customer-details page. This is the better UX and is closer to what CLAUDE.md's Customer Features list describes ("Add to Cart," "Update Cart"), but is a bigger behavior change than a strict "preserve existing functionality" reading allows silently — **flagged for your sign-off, see `MIGRATION_PLAN.md`.**
